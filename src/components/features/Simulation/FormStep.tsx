  ...
  return (
    <div className="bg-card rounded-2xl p-6 shadow-[4px_4px_18px_0px_rgba(0,0,0,0.2)] sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <Input {...inputProps}
          value={inputValue}
          onChange={(e) => setInputValue(inputProps.prefix === 'R$' ? formatCurrencyBRL(e.target.value) : e.target.value)} />
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-6">
          {!hideBackButton && (
            <Button
              ...
              variant="ghost"
              icon={ArrowLeft}
              className="order-2 flex-1 justify-center rounded-x1 py-3 sm:order-1">
              Voltar
            </Button>
          )}
          <Button type="submit"
